...
def run_nagrib2(nagrib2: str, grib: str, gemfile: str, maxgrd: int,
                g2tbls: str = "", overwrite: bool = False) -> str:
    """
    ...
    .gem file only works when that file's navigation matches -- which is the
    whole reason for the interpolation step.
    """
    script = NAGRIB2_TEMPLATE.format(
        gbfile=grib, gdoutf=gemfile, maxgrd=maxgrd,
        g2tbls=g2tbls, overwr="YES" if overwrite else "NO",
    )
    proc = subprocess.run([nagrib2], input=script, capture_output=True, text=True)
    text = proc.stdout + proc.stderr
    # nagrib2 exits 0 even when it writes nothing, so check the output text.
    if proc.returncode != 0 or "GRID" not in text.upper() and "Number of grids" not in text:
        if "error" in text.lower() or proc.returncode != 0:
            raise RuntimeError(f"nagrib2 failed for {grib}:\n{text.strip()}")
    if not os.path.exists(gemfile):
        raise RuntimeError(f"nagrib2 reported no error but {gemfile} was not created:\n{text.strip()}")
    return text
...
def main(argv: list[str] | None = None) -> int:
    ...
    g.add_argument("--clip", default="auto",
                   help="lonW:lonE:latS:latN to trim MRMS to before interpolating, "
                        "'auto' to derive it from the template (default), or 'none'")
    ...
    if not args.list_only:
        ...
        if new_grid and args.clip not in (None, "none"):
            if args.clip == "auto":
                ...
                clip = args.clip
            if clip:
                log(f"clipping MRMS to {clip} before interpolating")

    nagrib2 = None
    if not (args.list_only or args.no_gempak):
        gemexe = os.environ.get("GEMEXE")
        hint = ("Source your GEMPAK environment first (e.g. "
                ". $NAWIPS/Gemenviron.profile), or pass --nagrib2 /path/to/nagrib2, "
                "or use --no-gempak to stop after the GRIB2 stage.")
        cand = args.nagrib2 or (os.path.join(gemexe, "nagrib2") if gemexe else "nagrib2")
        nagrib2 = find_exe(cand, None, hint)
